namespace DataTableExtensions.Aggregation
{
    ...
    [DisplayName("Avg")]
    public class AvgActivity : CodeActivity
    {
        [Category("Input")]
        [RequiredArgument]
        ...
        [Category("Input")]
        [RequiredArgument]
        [Description("Column name to Avg")]
        ...
        [DefaultValue(EnumAvg.Select)]
        public EnumAvg ConvertInputColumn { get; set; } = EnumAvg.Select;
        ...
        private void DoAvg()
        {
            ...
            string strColumn;
            if (ConvertInputColumn != EnumAvg.Select)
            {
                this.CopayData();
            ...
            localOutPut = localDataTable.Compute("Avg(" + strColumn + ")", this.localSelect);
        ...
        {
            if (this.ConvertInputColumn == EnumAvg.Integer)
            {
                this.tempColumnType = System.Type.GetType("System.Int64");
            }
            else if (this.ConvertInputColumn == EnumAvg.Double)
            {
